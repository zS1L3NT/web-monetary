				...
				<Select
					...
					onChange={category => {
						if (category?.value && category.value !== "back") {
							...
							if (
								categories.find(c => c.id === category.value)!.category_ids.length >
								0
							) {
							...
							setParentCategoryId(
								categories.find(c => c.category_ids.includes(parentCategoryId!))
									?.id ?? null
							)
						}
					}}
						...
						Option: props => (
							<div data-cy="category-select-option">
								<components.Option {...props}>
									...
									) : (categories.find(c => c.id === props.data.value)
											?.category_ids.length ?? 0) > 0 ? (
										<Flex
											sx={{
